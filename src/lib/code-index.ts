 ...
 * @example
 * ```ts
 * dedupeCodeChunksByIdentity(await chunkProjectFiles(files));
 * ```
 */
 ...
 * @example
 * ```ts
 * await upsertProjectCodeChunks(projectId, await chunkProjectFiles(files));
 * ```
 */
...
export async function reindexProjectFiles(
  ...
  const chunks = dedupeCodeChunksByIdentity(await chunkProjectFiles(files));
