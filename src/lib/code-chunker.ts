...
const TREE_SITTER_RESOURCE_DIR = path.join(
  process.cwd(),
  "resources",
  "tree-sitter"
);
...
/**
 * Initializes the web-tree-sitter runtime once per process.
 *
 * Loads `tree-sitter.wasm` from the vendored resources directory so Next
 * standalone / serverless images do not depend on native addons.
 */
async function ensureWebTreeSitterReady(): Promise<boolean> {
  ...
  if (!parserInitPromise) {
    parserInitPromise = Parser.init({
      locateFile(scriptName: string) {
        return path.join(TREE_SITTER_RESOURCE_DIR, scriptName);
      },
    }).catch((error: unknown) => {
  ...
  try {
    ...
    console.error(
      "web-tree-sitter init failed; using line fallback:",
      error
    );
    return false;
  }
 ...
 *
 * Parsers are cached for the process lifetime. If WASM tree-sitter fails to
 * load, returns `null` so callers can use the line-window fallback without
 * crashing the index job.
 *
 ...
 * @example
 * ```ts
 * const splitter = await getAstCodeSplitter("tsx");
 * const parts = splitter?.splitText(source) ?? [];
 * ```
 */
async function getAstCodeSplitter(
  language: AstLanguage
): Promise<CodeSplitter | null> {
  if (!(await ensureWebTreeSitterReady())) {
    return null;
  }
  ...
  try {
    if (language === "tsx") {
      if (!tsxSplitter) {
        ...
        const lang = await Parser.Language.load(
          path.join(TREE_SITTER_RESOURCE_DIR, "tree-sitter-tsx.wasm")
        );
    ...
    if (!typescriptSplitter) {
      const parser = new Parser();
      const lang = await Parser.Language.load(
        path.join(TREE_SITTER_RESOURCE_DIR, "tree-sitter-typescript.wasm")
      );
      parser.setLanguage(lang);
    ...
    treeSitterLoadFailed = true;
    console.error(
      "web-tree-sitter CodeSplitter unavailable; using line fallback:",
      error
    );
    return null;
  }
 ...
 * @example
 * ```ts
 * await chunkFileWithAstSplitter("src/button.tsx", source, "tsx");
 * ```
 */
async function chunkFileWithAstSplitter(
  filePath: string,
  fileContent: string,
  language: AstLanguage
): Promise<CodeFileChunk[] | null> {
  const splitter = await getAstCodeSplitter(language);
  if (!splitter) {
    return null;
  }
 ...
 *
 * Prefer LlamaIndex CodeSplitter + web-tree-sitter for TS/JS. Falls back to
 ...
 * @example
 * ```ts
 * await chunkProjectFile("src/app/page.tsx", pageSource);
 * ```
 */
export async function chunkProjectFile(
  filePath: string,
  fileContent: string
): Promise<CodeFileChunk[]> {
  const normalizedPath = normalizeProjectPath(filePath);
  ...
  if (language) {
    const astChunks = await chunkFileWithAstSplitter(
      normalizedPath,
      content,
      language
    );
 ...
 * @example
 * ```ts
 * const chunks = await chunkProjectFiles(fragment.files);
 * ```
 */
export async function chunkProjectFiles(files: {
  [path: string]: string;
}): Promise<CodeFileChunk[]> {
  ...
  for (const filePath of paths) {
    ...
    allChunks.push(...(await chunkProjectFile(filePath, files[rawPath] ?? "")));
  }
