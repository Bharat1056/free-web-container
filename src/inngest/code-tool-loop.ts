...
/**
 * Builds the edit-context message from hybrid-retrieved code snippets.
 *
 * Always includes a full file tree so the model can call `readFiles` for
 * anything not inlined. Relevant snippets are listed in rank order.
 *
 * @param input.allPaths - Every project file path (sorted)
 * @param input.selectedSnippets - Ranked code pieces to inline
 * @param input.omittedPaths - Paths not inlined; model should readFiles these
 * @returns A user message instructing a minimal, grounded edit
 */
function buildEditContextMessage(input: {
  allPaths: string[];
  ...
  const fileTree =
    input.allPaths.length > 0
      ? input.allPaths.map((filePath) => `- ${filePath}`).join("\n")
      : "- (no files)";

  const relevantBlocks = input.selectedSnippets.map((snippet) => {
    const lineLabel =
      snippet.startLine === snippet.endLine
        ? `line ${snippet.startLine}`
        : `lines ${snippet.startLine}-${snippet.endLine}`;
    return `\n----- FILE: ${snippet.path} (${lineLabel}) -----\n${snippet.content}\n`;
  });

  const omittedNote =
    input.omittedPaths.length > 0
      ? `\n\nThese existing files were not inlined. Use readFiles before editing them: ${input.omittedPaths.join(", ")}.`
      : "";

  return `CONTEXT: This is an EDIT to an existing project. The file tree and the most relevant current code are below. Treat these as the source of truth — this is the app as it exists right now.

FILE TREE:
${fileTree}

RELEVANT CODE:
${relevantBlocks.join("") || "(no snippets selected; use readFiles against the file tree)"}
${omittedNote}
...
export async function runCodeToolLoop(input: {
  ...
  modelIds: string[];
  /** Hybrid-retrieved code context for edit/continuation runs. */
  codeContext?: {
    allPaths: string[];
    selectedSnippets: Array<{
      path: string;
  ...
  if (Object.keys(files).length > 0 && input.codeContext) {
    messages.push({
      role: "user",
      content: buildEditContextMessage(input.codeContext),
    });
  }
